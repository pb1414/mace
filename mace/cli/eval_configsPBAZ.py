...
# Allow 'slice' objects to be unpickled
...
def run(args: argparse.Namespace) -> None:
    ...
    for batch in data_loader:
        ...
        if model.__class__.__name__ == "MACELES":
            les_energiesList.append(torch_tools.to_numpy(output["les_energy"]))
            qs = np.split(
    ...
    if model.__class__.__name__ == "MACELES":
        les_energies = np.concatenate(les_energiesList, axis=0)
        assert len(atoms_list) == len(les_energies)
    ...
    for i, (atoms, energy, forces) in enumerate(zip(atoms_list, energies, forces_list)):
        ...
        if model.__class__.__name__ == "MACELES":
            atoms.info[args.info_prefix + "les_energy"] = les_energies[i]
